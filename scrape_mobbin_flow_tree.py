...
load_dotenv(dotenv_path='.env.mobbin_scraper')
...
class FlowTreeItem(BaseModel):
    ...
    level: int = Field(default=0, description="Indentation level in the tree")
    parent: Optional[str] = Field(default=None, description="Parent flow title")
    children: List[str] = Field(default_factory=list, description="Child flow titles")
...
async def scrape_flows_for_app(app_data: Dict[str, str], browser: Browser, save_screenshots_path: str) -> List[Dict[str, Any]]:
    ...
    app_downloads_dir = os.path.join("downloads", app_name)
    app_extracted_dir = os.path.join("extracted", app_name)
    ...
    try:
        # Create a new browser context for this app
        try:
            browser_context = await browser.new_context()
            page = await browser_context.get_current_page()
            logger.info(f"Created browser context for app: {app_data['title']}")
        except Exception as context_error:
            logger.error(f"Error creating browser context: {str(context_error)}")
            # Try to recreate the browser
            try:
                await browser.close()
                logger.info("Closed browser to recreate it")
                ...
                browser = Browser(config=BrowserConfig(
                    ...
                    chrome_instance_path=os.environ.get('CHROME_PATH', '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'),
                    extra_chromium_args=[
                        f'--profile-directory={os.environ.get("CHROME_PROFILE", "Profile 4")}'
                    ]
                ))
                
                browser_context = await browser.new_context()
                page = await browser_context.get_current_page()
                logger.info("Successfully recreated browser and context")
            except Exception as browser_error:
                logger.error(f"Failed to recreate browser: {str(browser_error)}")
                return []
        ...
        flow_map: Dict[str, Dict[str, Any]] = {}  # Map of title to flow item
        ...
        if existing_flows and isinstance(existing_flows, list):
            ...
            flow_map = {item['title']: item for item in flow_items}
        else:
            ...
            try:
                ...
                logger.info(f"Found {len(flow_nodes)} flow nodes on page")
                
                # First pass: collect all nodes with their data
                for node in flow_nodes:
                    ...
                    # Calculate indentation level based on DOM structure
                    try:
                        # Primary approach: Use TreeNodeBranch components
                        branch_divs = await node.query_selector_all('div[data-sentry-component="TreeNodeBranch"]')
                        level = len(branch_divs)
                        logger.debug(f"Detected level {level} for '{title}' using branch divs")
                    except Exception as e:
                        logger.warning(f"Error detecting level using branch divs: {str(e)}")
                        try:
                            # Fallback approach: Use the data attribute directly
                            has_child = await node.get_attribute('data-tree-has-visible-child')
                            is_ancestor = await node.get_attribute('data-tree-is-ancestor-selected')
                            
                            # Count indentation divs manually
                            html = await node.evaluate('el => el.outerHTML')
                            indent_count = html.count('TreeNodeBranch')
                            level = indent_count
                            logger.debug(f"Detected level {level} for '{title}' using fallback method")
                        except Exception as e2:
                            # Last resort: use a simple heuristic based on the HTML structure
                            logger.warning(f"Fallback level detection also failed: {str(e2)}")
                            level = 0
                    
                    link = f"https://mobbin.com/flows/{node_id}?tab=prototype" if node_id else flows_url
                    
                    # Create flow item
                    flow_item = FlowTreeItem(
                        title=title,
                        link=link,
                        count=count,
                        level=level
                    ...
                    flow_map[title] = flow_item
                    
                    # For testing purpose
                    # if len(flow_items) == 10:
                    #     break
                
                # Second pass: establish parent-child relationships
                for flow_item in flow_items:
                    current_level = flow_item['level']
                    current_title = flow_item['title']
                    current_index = flow_items.index(flow_item)
                    
                    # Initialize children as empty list if not present
                    if 'children' not in flow_item:
                        flow_item['children'] = []
                    
                    if current_level > 0:
                        # Find parent (first item above this one with a lower level)
                        parent_item = None
                        for i in range(current_index - 1, -1, -1):
                            potential_parent = flow_items[i]
                        ...
                        if parent_item:
                            ...
                            flow_item['parent'] = parent_item['title']
                            
                            # Add this item to parent's children if not already there
                            if 'children' not in parent_item:
                                parent_item['children'] = []
                            
                            if current_title not in parent_item['children']:
                                parent_item['children'].append(current_title)
            ...
            while retry_count < max_retries:
                try:
                    ...
                    try:
                        ...
                        logger.info(f"Successfully navigated to flow: {flow_title}")
                    except Exception as e:
                        logger.warning(f"Page navigation failed, recreating context: {str(e)}")
                        # Close the old context if it exists
                        if browser_context:
                            try:
                                await browser_context.close()
                                logger.info("Closed problematic browser context")
                            except Exception as close_error:
                                logger.warning(f"Error while closing browser context: {str(close_error)}")
                        ...
                        try:
                            ...
                            await page.goto(flow['link'])
                            logger.info(f"Successfully navigated to flow with new context: {flow_title}")
                        except Exception as nav_error:
                            logger.error(f"Failed to navigate after context recreation: {str(nav_error)}")
                            retry_count += 1
                    ...
                    try:
                        ...
                        logger.warning(f"Error waiting for menu selector: {str(selector_error)}")
                        retry_count += 1
                    ...
                    try:
                        ...
                        logger.warning(f"Error waiting for download selector: {str(selector_error)}")
                        retry_count += 1
                        if retry_count < max_retries:
                            logger.info(f"Retrying ({retry_count}/{max_retries})...")
                            await asyncio.sleep(random.uniform(1.0, 3.0))
                    ...
                    try:
                        ...
                        logger.error(f"Error saving download: {str(save_error)}")
                        retry_count += 1
                        if retry_count < max_retries:
                            logger.info(f"Retrying save ({retry_count}/{max_retries})...")
                            await asyncio.sleep(random.uniform(1.0, 3.0))
                            continue
                        else:
                            break
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to capture screenshot: {str(screenshot_error)}")
                    ...
                    if retry_count < max_retries:
                        ...
                        # If the context seems closed, try to recreate it
        ...
        app_flows_file = os.path.join("mobbin_flows", f"{app_name}_flows.json")
        os.makedirs("mobbin_flows", exist_ok=True)
        ...
        # Make sure to close the browser context to free resources
        if browser_context:
            try:
                await browser_context.close()
                logger.info(f"Browser context closed for app: {app_data['title']}")
            except Exception as close_error:
                logger.warning(f"Error closing browser context: {str(close_error)}")
...
async def scrape_all_apps():
    save_screenshots_path = "./mobbin_screenshots"
    os.makedirs(save_screenshots_path, exist_ok=True)
    ...
    with open("mobbin_apps_complete.json", "r") as f:
    ...
    # Create a single browser instance for the entire process
    browser = None
    ...
    try:
        ...
        for index, app in enumerate(apps_to_process):
            ...
            try:
                # Make sure browser is active before processing app
                if browser is None:
                    logger.info("Browser is None, creating new browser instance")
                    browser = Browser(config=BrowserConfig(
                        new_context_config=BrowserContextConfig(
                            browser_window_size={'width': 1980, 'height': 1200}
                        ),
                ...
                if s3_bucket:
                    ...
                    flow_json_path = os.path.join("mobbin_flows", f"{app_name}_flows.json")
                ...
                logger.error(f"Error while processing app {app_name}: {str(app_error)}")
                logger.info("Continuing with the next app...")
                
                # Try to recreate the browser if it failed
                if "Target page, context or browser has been closed" in str(app_error):
                    logger.info("Recreating browser due to context error...")
                    try:
                        if browser:
                            await browser.close()
                    except Exception as close_error:
                    ...
                    try:
                        ...
                        logger.info("Successfully recreated browser")
                    except Exception as browser_error:
                        logger.error(f"Failed to create new browser: {str(browser_error)}")
                        browser = None
            
            # Add delay between apps to avoid rate limiting
            if index < len(apps_to_process) - 1 and browser is not None:  # Don't delay after the last app
        ...
        # Make sure to close the browser in any case
        if browser:
            try:
                await browser.close()
